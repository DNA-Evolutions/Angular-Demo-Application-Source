...
export class LoadExampleDataService {
    ...
    public extractEdgeConnections(ids: string[]): JOptEdgeConnection[] {
        return this.geoConnections.filter(c => this.isDesiredConnectionsMember(ids, c));
    }

    private isDesiredConnectionsMember(ids: string[], c: JOptEdgeConnection): boolean {
    ...
    public async loadExample(exampleId: string): Promise<unknown> {
        ...
        return this.loadExampleHelper(environment.exampleAssertHelperPath).then(() => {
