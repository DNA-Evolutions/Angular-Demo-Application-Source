...
import { JOptGeoNode, JOptRoute } from 'build/openapi';
...
export class LeafletPolylineService {
    ...
    constructor(private geoService: GeoAndRoutingService, private popupService: PopUpService) { }

    public drawRouteResultPolyline(route: JOptRoute, index: number, ref: ElementRef, map: any): void {
    ...
    private drawMutliElementPolyine(ids: Array<string>, route: JOptRoute, index: number, ref: ElementRef, map: any): void {
        ...
        // for (let ii = 0; ii < numIds; ii++) {

        //     if (ii === 0 || ii === numIds - 1) {
        //         idsForCorrdinates.push(ids[ii]);
        //     } else {
        //         idsForCorrdinates.push(ids[ii]);
        //         idsForCorrdinates.push(ids[ii]);
        //     }
        ...
        for (let ii = 0; ii < shapes.length; ii++) {
            const ployline = L.polyline(shapes[ii], { color: this.getColor(index), smoothFactor: 1, opacity: 0.6, weight: 5 });
    ...
    // public drawPolyline(elementIdFrom: string, elementIdTo: string, map: any): void {

    //     //console.log('drawPolyline');

    //     const polyline = this.geoService.getSingleRouteShape(elementIdFrom, elementIdTo);
